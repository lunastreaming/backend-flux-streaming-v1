...
public class WalletService {
    ...
    public void cancelPendingRecharge(String principalName, UUID txId) {
        WalletTransaction tx = walletTransactionRepository.findById(txId)
                .orElseThrow(() -> FluxException.notFound("Transacción no encontrada"));


        if (!"pending".equalsIgnoreCase(tx.getStatus())) {
            throw FluxException.invalidState("Solo transacciones en estado pending pueden cancelarse");
        }
        ...
        if (!isOwner && !isAdmin) {
            throw FluxException.accessDenied("No autorizado para cancelar esta transacción");
        }
